...
let playerXP = 0; ///Fehrler behoben                   // Stellt die gesammelte Erfahrung des Spielers dar.
...
let prefix = ["Wald-", "Seuchen-", "Uralte(s) ", "Gift-", "Brennende(s) ", "Kniescheibenzertrümmernde(s) ", "Gemeine(s)", "Öko", "Vegane(s)", "Fluss-", "Haus-"]; // length = 6, da 6 Einträge. Von 0-5.
let monsterName = ["Ratte", "Nagetier", "Ungeziefer", "Monster", "Ding", "Bestie", "Vieh", "Ungeheuer"]; // length = 3, da 3 Einträge. Von 0-2.
let suffix = [" des Verderbens", " aus der Hölle", " der Lethalität", " mit Rheuma", " der Redundanz", " der Zerberstung", " der Highlands", " des indischen Ozeans", " von Neben an", " mit Hunger", " mit Aggressionsproblem"]; // length = 6, da hier 6 Einträge sind. Von 0-5.
...
let monsterSnack2 = ["Butter Toast", "Toast Hawaii", "French Toast", "Schinken Käse Toast"];
let monsterBildSrc = ["imgs/monster1.jpg", "imgs/monster2.jpg", "imgs/monster3.jpg"];
...
window.onload = function () {
    ...
    document.getElementById("weakmonsterfighter").addEventListener("click", fightAllWeakMonsters, false);
    document.getElementById("weakestmonsterfighter").addEventListener("click", fightWeakestMonster, false);
};
...
function generateMonster() {
    let monsterAnzahl = getRNGNumber(3) + 1; // eine zahl zwischen eins und 3 wird generiert
    for (let i = 0; i < monsterAnzahl; i++) { //die generierung der monster wird so oft ausgeführt, bis die zufalls generierte zahl von oben nicht mehr größer ist als die anzahl der generierungen
        ...
        let newMonsterSnack = generatemonsterSnack();
...
function monsterGenerateHTML(Länge = monsterArray.length) {
    ...
    monsterName.innerHTML = monsterArray[Länge].monsterName; // Inhalt des <p>: Monster-Name des letzten Monsters im Array.
    holdingDiv.appendChild(monsterName); // Füge das <p> zum HTML-Dokument hinzu, indem es dem holding-Div angefügt wird.
    let monsterMod = document.createElement("p"); // Generiere einen <p>
    monsterMod.innerHTML = monsterArray[Länge].monsterModifier[0] + ", " + monsterArray[Länge].monsterModifier[1]; // Inhalt des <p>: Monster-Modifizierer null und eins
    holdingDiv.appendChild(monsterMod); // Füge das <p> zum HTML-Dokument hinzu, indem es dem holding-Div angefügt wird.
    let monsterSnack = document.createElement("p"); // Generiere einen <p>
    monsterSnack.innerHTML = "isst gern " + monsterArray[Länge].monsterSnack;
    holdingDiv.appendChild(monsterSnack);
    let monsterImg = document.createElement("img"); // Erstelle ein <img>-Element
    monsterImg.setAttribute("src", monsterArray[Länge].monsterBild); // Der Pfad für das Bild muss über setAttribute festgelegt werden. Der Bildpfad kann natürlich auch anders aussehen.
    monsterImg.setAttribute("alt", "Schreckliches Monster"); // Das alt für das Bild wird hier festgelegt.
    holdingDiv.appendChild(monsterImg);
    let monsterHealthPoints2 = document.createElement("p");
    monsterHealthPoints2.innerHTML = "Health Points: " + monsterArray[Länge].monsterHealthPoints;
    holdingDiv.appendChild(monsterHealthPoints2);
    let monsterExperience2 = document.createElement("p");
    monsterExperience2.innerHTML = "Experience: " + monsterArray[Länge].monsterExperience;
    holdingDiv.appendChild(monsterExperience2);
    let monsterLevel2 = document.createElement("p");
    monsterLevel2.innerHTML = "Monster-Level: " + monsterArray[Länge].monsterLevel;
    holdingDiv.appendChild(monsterLevel2);
    let monsterBtn = document.createElement("BUTTON"); // Erstelle ein <button>-Element
    monsterBtn.innerHTML = "Monster bekämpfen!!!"; // Verändere den Inhalt des HTML-Elementes. Der genaue Text ist dabei euch überlassen.
    holdingDiv.appendChild(monsterBtn); // Füge den Button zu dem holding-div hinzu.
    let monsterCount = Länge; // Die aktuelle Anzahl vorhandener Monster, zudem auch die neue Zahl für das Monster-Array.
...
function generateMonsterBild() {
    let rngNumber = getRNGNumber(monsterBildSrc.length);
    let generatedMonsterImage = monsterBildSrc[rngNumber];
...
function generateMonsterXP() {
    ...
    let tempMonsterXP = 100 + getRNGNumber(700);
...
function generatemonsterSnack() {
    let tempMonsterSnack;
    tempMonsterSnack = monsterSnack2[getRNGNumber(monsterSnack2.length)];
    return tempMonsterSnack;
}
// Aufgerufen, wenn man auf den Button klickt.
// Der Spieler kämpft gegen das entsprechende Monster. Er erhält dann Erfahrungspunkte.
function fightMonster(_index = monsterArray.length) {
